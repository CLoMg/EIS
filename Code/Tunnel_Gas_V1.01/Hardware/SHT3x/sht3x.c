...
void SHT30_Init(void)
{
	...
	uint8_t i=0;
	char Init_Info[]="IIC_1 SHT30 Init Success\r\n";
	#if hardwareiic
	//I2Cx_Init(I2C1);
	#else
	IIC_Init(sht_iic_num);
	#endif
	if(!SHT30_Acquire(0x44,dummy_buff))
		memcpy(Init_Info,"IIC_1 SHT30 Init Failed\r\n",sizeof("IIC_1 SHT30 Init Failed\r\n"));
...
Error_Code_t SHT30_Acquire (u8 slave_addr,uint8_t *data_buff){
	...
	uint8_t count=5;
	while((!(SHT30_read_result(slave_addr,read_buff)))&&((count--)>0));
	...
	if((Temperature>=-20)&&(Temperature<=125)&&(Humidity>=0)&&(Humidity<=100))//过滤错误数据
{
		...
		*(read_buff++)=(uint8_t)((uint16_t)Temperature);
		*(read_buff++)=(uint8_t)((uint16_t)Humidity>>8);
		*(read_buff++)=(uint8_t)((uint16_t)Humidity);
	  Error=1;
	}
	else
			Error=0;
